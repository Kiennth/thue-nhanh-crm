-- CEO 2026-08-04: "chuyển lịch sử thuê của iPhone 13 Pro Max (SL) sang
-- iPhone 13 Pro Max để theo dõi theo serial". Deposit đổi 5.000.000đ ->
-- 2.000.000đ, 2 đơn đang mở — đóng băng cọc dự kiến (17.000.000đ và
-- 8.000.000đ) trước khi đổi loại sản phẩm.
do $$
declare
  v_old_type_id uuid;
  v_new_type_id uuid;
  ...
  i integer;
begin
  select id into v_old_type_id from public.equipment_types where name = 'iPhone 13 Pro Max (SL)';
  select id into v_new_type_id from public.equipment_types where name = 'iPhone 13 Pro Max';
  ...
  values (v_new_type_id, 'IPHONE13PROMAX-01', 'available')
