-- CEO 2026-08-04: "chuyển PC Core i5 14th 32GB RAM RTX 3060 qua theo
-- serial nhé" — chuyển từ quantity-tracked sang individual. 6 dòng
-- order_equipment đã gắn sẵn (5 dòng quantity>1), 20.440.000đ, 0 đơn đang
-- mở nên không cần đóng băng cọc. equipment_stock thật: 10/10/10 theo
-- Hà Nội/TP HCM/Đà Nẵng — tạo instance đúng theo tồn kho thật.
do $$
declare
  v_type_id uuid;
  v_unit_id uuid;
  ...
  v_pos integer;
begin
  select id into v_type_id from public.equipment_types where name = 'PC Core i5 14th 32GB RAM RTX 3060';
  ...
  for i in 1..10 loop
    ...
    values (v_type_id, v_branch_hn, 'PCI5-14TH-3060-HN-' || lpad(i::text, 2, '0'), 'available')
    returning id into v_inst_id;
    v_hn_ids := array_append(v_hn_ids, v_inst_id);
  end loop;
  for i in 1..10 loop
    insert into public.equipment_instances (equipment_type_id, branch_id, identifier_code, status)
    values (v_type_id, v_branch_hcm, 'PCI5-14TH-3060-HCM-' || lpad(i::text, 2, '0'), 'available')
    returning id into v_inst_id;
    v_hcm_ids := array_append(v_hcm_ids, v_inst_id);
  end loop;
  for i in 1..10 loop
    insert into public.equipment_instances (equipment_type_id, branch_id, identifier_code, status)
    values (v_type_id, v_branch_dn, 'PCI5-14TH-3060-DN-' || lpad(i::text, 2, '0'), 'available')
    returning id into v_inst_id;
