-- CEO 2026-08-04: sau migration 20260804260000, CEO đổi tên type gộp
-- (theo số lượng) trở lại "iPhone 15 Pro Max (SL)" và tạo type mới
-- "iPhone 15 Pro Max" (theo từng sản phẩm) để serialize thật — yêu cầu
-- lại "chuyển lịch sử thuê của iPhone 15 Pro Max (SL) sang iPhone 15
-- Pro Max bên CRM mới" lần 2 cho đúng type mới này. Deposit không đổi
-- (5.000.000đ cả 2 bên), 0 đơn đang mở.
do $$
declare
  v_old_type_id uuid;
  v_new_type_id uuid;
...
begin
  ...
  where name = 'iPhone 15 Pro Max (SL)' and tracking_type = 'quantity';
  select id into v_new_type_id from public.equipment_types
  where name = 'iPhone 15 Pro Max' and tracking_type = 'individual';
  ...
  values (v_new_type_id, 'IPHONE15PROMAX-01', 'available')
