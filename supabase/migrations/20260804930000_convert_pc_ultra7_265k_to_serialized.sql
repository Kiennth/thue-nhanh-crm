-- CEO 2026-08-04: "PC Ultra7 265K | 64GB RAM | RTX 4070 12GB chuyển qua
-- serial dùm tao" — chuyển từ quantity-tracked sang individual. 4 dòng
-- order_equipment đã gắn sẵn (toàn quantity=1), 13.155.000đ, 0 đơn đang
...
begin
  ...
  for i in 1..10 loop
    ...
    values (v_type_id, v_branch_hn, 'PCULTRA7-265K-HN-' || lpad(i::text, 2, '0'), 'available')
    returning id into v_inst_id;
    v_hn_ids := array_append(v_hn_ids, v_inst_id);
  end loop;
  for i in 1..10 loop
    insert into public.equipment_instances (equipment_type_id, branch_id, identifier_code, status)
    values (v_type_id, v_branch_hcm, 'PCULTRA7-265K-HCM-' || lpad(i::text, 2, '0'), 'available')
    returning id into v_inst_id;
    v_hcm_ids := array_append(v_hcm_ids, v_inst_id);
  end loop;
  for i in 1..10 loop
    insert into public.equipment_instances (equipment_type_id, branch_id, identifier_code, status)
    values (v_type_id, v_branch_dn, 'PCULTRA7-265K-DN-' || lpad(i::text, 2, '0'), 'available')
    returning id into v_inst_id;
