-- CEO 2026-09-02 (tiếp 20260902100000): thanh "Tiến độ thu tiền" phải cộng
-- đúng bằng "Tổng doanh số" — cả 2 cùng nền ĐƠN ĐÃ GIAO, gồm VAT. Thêm
-- deliveredUnpaidAmount (còn thiếu của riêng đơn đã giao) cho thanh; cặp
-- unpaidCount/unpaidAmount (mọi đơn chưa huỷ) giữ nguyên cho thẻ "Chưa
-- thanh toán hết" + trang Công nợ — nợ đơn đặt trước không mất dấu.
-- Chỉ thay orders_page_list; thân hàm giữ nguyên bản 20260902100000, khác
-- mỗi khối stats.
...
select jsonb_build_object(
  ...
  'stats', (
    select jsonb_build_object(
      ...
      -- (totalRevenue, deliveredUnpaidAmount) nên Đã thu + Còn thiếu luôn
      -- cộng đúng bằng Tổng doanh số.
