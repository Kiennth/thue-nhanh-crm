-- ---------------------------------------------------------------------
-- CEO yêu cầu 2026-08-06: thêm "Toàn thời gian" vào toggle kỳ của báo cáo
-- khách hàng — donut "Tỉ trọng số khách hàng" ở kỳ này sẽ bao quát luôn 2
-- ô "Tổng khách hàng"/"Cá nhân-Doanh nghiệp" phía trên (đang tính từ
-- stats/scoped_cust_agg — KHÔNG cùng nền với donut, vốn chỉ đếm khách CÓ
-- ĐƠN trong kỳ). Lưu ý: 2 số này KHÔNG khớp tuyệt đối (5.540 khách đăng ký
-- vs 4.670 khách có đơn — 870 khách chưa từng thuê) — chấp nhận vì "khách
-- có đơn" là số có ý nghĩa vận hành hơn.
--
-- Thêm dòng ('allTime', null, null) vào period_defs — start_date/end_date
-- null nghĩa "không giới hạn", period_orders/period_orders_typed/
-- period_new_customers đổi điều kiện join sang "cột null hoặc so sánh"
-- (giống pattern p_range_start/p_range_end is null or ... đã dùng ở
-- orders_page_list) thay vì so sánh trực tiếp với date không thể null.
-- ---------------------------------------------------------------------
