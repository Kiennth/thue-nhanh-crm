-- CEO 2026-08-04: "chuyển lịch sử thuê của iPhone 11 Pro Max (SL) sang
-- iPhone 11 Pro Max để theo dõi theo serial". Deposit đổi 0đ ->
-- 2.000.000đ nhưng 0 đơn đang mở nên an toàn.
do $$
declare
  v_old_type_id uuid;
  v_new_type_id uuid;
  ...
  i integer;
begin
  select id into v_old_type_id from public.equipment_types where name = 'iPhone 11 Pro Max (SL)';
  select id into v_new_type_id from public.equipment_types where name = 'iPhone 11 Pro Max';
  ...
  values (v_new_type_id, 'IPHONE11PROMAX-01', 'available')
