-- CEO 2026-08-04: "chuyển lịch sử thuê của iPhone 13 bên Booqable sang
-- iPhone 13 bên CRM mới" — đối chiếu Booqable API xác nhận đúng 7 đơn,
-- 53 SL, khớp 100% với dữ liệu đã gắn "iPhone 13 (SL)" bên CRM.
-- Deposit đổi 0đ -> 1.000.000đ, có 1 đơn đang mở (BQ11586, 35 máy) —
-- đóng băng cọc dự kiến 10.000.000đ cho đơn này trước khi đổi loại.
do $$
declare
  v_old_type_id uuid;
  v_new_type_id uuid;
  ...
  i integer;
begin
  select id into v_old_type_id from public.equipment_types where name = 'iPhone 13 (SL)';
  select id into v_new_type_id from public.equipment_types where name = 'iPhone 13';
  ...
  values (v_new_type_id, 'IPHONE13-01', 'available')
