-- CEO 2026-08-04: "chuyển lịch sử thuê của iPhone 11 (SL) sang iPhone
-- 11" — deposit đổi từ 0đ lên 1.000.000đ nhưng 0 đơn đang mở nên an
-- toàn, không cần đóng băng cọc.
do $$
declare
  v_old_type_id uuid;
  v_new_type_id uuid;
  ...
  i integer;
begin
  select id into v_old_type_id from public.equipment_types where name = 'iPhone 11 (SL)';
  select id into v_new_type_id from public.equipment_types where name = 'iPhone 11';
  ...
  values (v_new_type_id, 'IPHONE11-01', 'available')
