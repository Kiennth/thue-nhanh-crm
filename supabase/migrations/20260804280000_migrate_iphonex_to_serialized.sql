-- CEO 2026-08-04: "chuyển lịch sử thuê của iPhone X (SL) sang cho
-- iPhone X bên CRM mới" — deposit đổi 0đ -> 500.000đ nhưng 0 đơn đang
-- mở nên an toàn.
do $$
declare
  v_old_type_id uuid;
  ...
  i integer;
begin
  select id into v_old_type_id from public.equipment_types where name = 'iPhone X (SL)';
  select id into v_new_type_id from public.equipment_types where name = 'iPhone X';
  ...
  values (v_new_type_id, 'IPHONEX-01', 'available')
