-- ---------------------------------------------------------------------
-- CEO yêu cầu 2026-08-06: "phần báo cáo khách hàng cũng cho toggle tháng,
-- năm giống phần đơn hàng" — thay 3 thẻ công nợ tĩnh (thisMonth/lastMonth/
-- thisYear, thêm ở 20260806110000) bằng khối tổng quan có toggle Tháng
-- này/Tháng trước/Năm nay/Năm trước, giống hệt UX orders_page_list.
--
-- Đổi field trả về: bỏ "debtByPeriod" (3 key cố định), thay bằng
-- "periodStats" — object khoá theo tên kỳ (thisMonth/lastMonth/thisYear/
-- lastYear), mỗi kỳ có 3 số: newCustomers (khách mới CỦA CÔNG TY có đơn
-- đầu tiên rơi vào kỳ, tôn trọng branch-scoping y hệt monthly_new sẵn có),
-- revenue (doanh thu đã gồm VAT của các đơn trong kỳ), debt (công nợ còn
-- thiếu — công thức y hệt debt_by_period cũ).
-- ---------------------------------------------------------------------
...
-- 4 kỳ cho khối tổng quan có toggle (Tháng này/Tháng trước/Năm nay/Năm
-- trước) — cùng cách tính mốc giờ VN như "months" ở trên.
period_defs as (
  select * from (values
    ('thisMonth'::text,
...
period_orders as (
  ...
  from period_defs pd
  join scoped s on s.order_date >= pd.start_date and s.order_date <= pd.end_date
),
...
-- Khách MỚI CỦA CÔNG TY (không phải mới của riêng chi nhánh) có đơn đầu
-- tiên rơi vào kỳ — cùng điều kiện branch-scoping với monthly_new
-- (ca.first_order = ca.company_first_order: chi nhánh đang xem đúng là nơi
-- khách phát sinh đơn đầu tiên).
period_new_customers as (
  ...
  join cust_agg_cf ca
    on ca.first_order is not null
   and ca.first_order = ca.company_first_order
   and ca.first_order >= pd.start_date and ca.first_order <= pd.end_date
  group by pd.period
)
select case
  when not exists (select 1 from caller) then
    ...
    'periodStats', (
      select coalesce(jsonb_object_agg(pd.period, jsonb_build_object(
        'newCustomers', coalesce(pnc.cnt, 0),
        'revenue', coalesce(prd.revenue, 0),
        'debt', coalesce(prd.debt, 0)
      )), '{}'::jsonb)
      from period_defs pd
      left join period_new_customers pnc on pnc.period = pd.period
      left join period_revenue_debt prd on prd.period = pd.period
    )
  )
end
$$;
